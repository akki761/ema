 ...
 *
 * The homepage template has 7 sections (page-templates.json). This transformer
 * inserts an <hr> before each non-first section and a "Section Metadata" block
 * for each section carrying a `style` (rc1/rc3/rc5 → "secondary").
 *
 * Section selectors come directly from page-templates.json (DOM-verified during
 * page analysis) via payload.template.sections — not re-derived here.
 *
 * Breaks are inserted in beforeTransform (while every section element still
 * exists, before block parsers replace them) using a marker attribute; metadata
